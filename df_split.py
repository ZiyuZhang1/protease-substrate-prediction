...
negdf = negative_data.sample(int(len(df_train)/2))
...
neg_from2 = df = pd.read_csv(folderlist[another])
neg_from2 = neg_from2.sample(n=int(len(df_train)/2))
neg_from2['label'] = [0]*len(neg_from2)
negdf = pd.concat([negdf,neg_from2])
