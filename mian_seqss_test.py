...
# neg = negdf.sample(n=len(test1),random_state=42)
neg = negdf.sample(n=len(test1)*10,random_state=42)

# set1 = set(test1['peptide'].tolist())&set(test2['peptide'].tolist())
### get test data
all_test1 = pd.concat([test1, neg])
all_test2 = pd.concat([test2, neg])
